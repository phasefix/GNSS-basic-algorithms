...
def sate_pos_ap2(t, eph):
    ...
    R3_minus_lmbda_ecef = np.array([
        [np.cos(lambda_ecef), -np.sin(lambda_ecef), 0],
        [np.sin(lambda_ecef), np.cos(lambda_ecef), 0],
        [0, 0, 1]
    ])
    ...
    X, Y, Z = R3_minus_lmbda_ecef @ R1_minus_i @ pos_orb

    # velocity in orbital plane (not required for position, but useful for completeness)
    vx_orb = -n*A**2 * np.sin(E) / r
    vy_orb = n*A**2 * np.sqrt(1 - e**2) * np.cos(E) / r
    vz_orb = 0  # Assuming no velocity component in the z-direction
    vel_orb = np.array([vx_orb, vy_orb, vz_orb])
    
    vx, vy, vz = R3_minus_lmbda_ecef @ R1_minus_i @ vel_orb
